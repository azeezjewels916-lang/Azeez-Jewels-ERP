...
function getBarcodeSvgString(text: string): string {
  try {
    ...
    JsBarcode(svgNode, text || 'AHS000000', {
      format: "CODE128",
      width: 1.5, // Thicker bars to help laser scanners
      height: 50, // Taller bars are much easier to scan
      displayValue: false,
      margin: 12, // CRITICAL: Scanners NEED a massive white quiet zone on both sides!
      background: "#ffffff", // Pure white background to guarantee quiet zone is white
      lineColor: "#000000"
    });
    
    // Let the browser scale it smoothly. The huge internal margin guarantees 
    // the quiet zone is respected regardless of CSS width.
